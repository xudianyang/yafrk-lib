...
namespace Yafrk\Mail\Transport;

use Yafrk\Mail\Address;
use Yafrk\Mail\Headers;
use Yafrk\Mail\Message;
use Yafrk\Mail\Protocol;
use Yafrk\Mail\Protocol\Exception as ProtocolException;
...
//        $connection       = $this->plugin($options->getConnectionClass(), $config);
        ...
        {
            case 'smtp':
                $this->connection = new  \Yafrk\Mail\Protocol\Smtp($config);
                break;
            case 'plain':
            case 'login':
            case 'crammd5':
                $class = "\\Yafrk\\Mail\\Protocol\\Smtp\\Auth\\" . ucfirst($options->getConnectionClass());
